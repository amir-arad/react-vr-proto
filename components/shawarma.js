...
export default class Shawarma extends React.Component {
  ...
  rotateModel() {
    this.state.rotateYValue.setValue(
      this.state.rotateYValue.toJSON() % FULL_CIRCLE_VALUE
    ); // Start large
    this.animation = Animated.timing(
      ...
      this.state.rotateYValue, // Animate `bounceValue`
      {
        ...
        duration: FULL_CIRCLE_TIME *
          (FULL_CIRCLE_VALUE - this.state.rotateYValue.toJSON()) /
          FULL_CIRCLE_VALUE
      }
    );
